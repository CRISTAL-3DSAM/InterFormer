...
def init_vars(src, model,init_pos, opt):
    ...
    #out1 = model.out(model.decoder(outputs,e_output, src_mask, trg_mask))
    out = init_pos
    ...
    if opt.device == 0:
        outputs = outputs.cuda()
    outputs[:, 0,:] = torch.from_numpy(init_tok).transpose(1,0) #commenté
    outputs[:, 1,:] = out[:,:,1] # modifie
...
def beam_search(src, model,init_pos, opt):
    ...
    ind = None
    att_dec = {}
    for i in range(2, opt.max_len): # modifie
