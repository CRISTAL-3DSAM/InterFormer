...
def train_model(model, opt):
    ...
    if opt.eval:
        ...
        #train_select = np.asarray([i for i in range(nb_samples) if i not in eval_select])
        #input_train = opt.data_input[train_select,:,:]
        #output_train = opt.data_output[train_select,:,:]
    ...
    for epoch in range(opt.epochs):
        ...
        for i in range(nb_batch):
            ...
            total_loss += loss.item()+loss_ff_print
            ...
            if (i + 1) % opt.printevery == 0:
                 ...
                 avg_loss = loss.item()
                 if opt.floyd is False:
                    print("   %dm: epoch %d [%s%s]  %d%%  loss = %.3f  loss_train = %.3f loss_ff =%.3f " %\
                    ((time.time() - start)//60, epoch + 1, "".join('#'*(p//5)), "".join(' '*(20-(p//5))), p, avg_loss, loss,loss_ff_print), end='\r')
                 else:
                    print("   %dm: epoch %d [%s%s]  %d%%  loss = %.3f  loss_train = %.3f loss_ff =%.3f " %\
                    ((time.time() - start)//60, epoch + 1, "".join('#'*(p//5)), "".join(' '*(20-(p//5))), p, avg_loss,loss,loss_ff_print))
        ...
        print("%dm: epoch %d [%s%s]  %d%%  loss = %.3f\nepoch %d complete, loss = %.03f  loss_train = %.3f loss_ff =%.3f " %\
        ((time.time() - start)//60, epoch + 1, "".join('#'*(100//5)), "".join(' '*(20-(100//5))), 100, avg_loss, epoch + 1, avg_loss, total_loss_train/nb_batch,total_loss_ff/nb_batch))
        total_loss = 0
        if epoch%100==0:
            dst= 'temp_save'
        ...
        if epoch>max_normal_epoch and epoch%1 and  opt.eval:
            dst= 'temp_save'
            print("saving weights to " + dst + "/...")
...
def main():
    ...
    parser.add_argument('-max_len', type=int, default=52) # ajouté pour test, à intégrer dans le code
